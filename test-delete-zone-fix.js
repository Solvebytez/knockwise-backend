...
async function testDeleteZoneLogic() {
  try {
    // Connect to MongoDB
    await mongoose.connect(process.env.MONGODB_URI);
    console.log('Connected to MongoDB');

    console.log('\n=== TESTING DELETE ZONE LOGIC ===\n');
    ...
    // 3. Execute the same logic that happens after zone deletion
    console.log('\n3. EXECUTING DELETE ZONE LOGIC:');
    
    // Get all teams that might be affected
    const allTeams = await Team.find({});
    
    for (const team of allTeams) {
      console.log(`\nProcessing team: ${team.name} (${team._id})`);
      
      // Update team assignment status (same logic as in deleteZone)
      await updateTeamAssignmentStatus(team._id.toString());
      
      // Update team status (same logic as in deleteZone)
