...
const updateTeamAssignmentStatus = async (teamId) => {
    try {
        ...
        if (newAssignmentStatus !== team.assignmentStatus) {
            ...
            await Team_1.Team.findByIdAndUpdate(teamId, {
...
async function listAssignments(_req, res) {
    const list = await AgentZoneAssignment_1.default.find()
        .sort({ createdAt: -1 })
        .limit(200);
    res.json(list);
}
async function getMyAssignments(req, res) {
    try {
        ...
        const assignments = await AgentZoneAssignment_1.default.find({ agentId }).sort({
            createdAt: -1,
        });
...
async function getTeamAssignments(req, res) {
    try {
        ...
        const assignments = await AgentZoneAssignment_1.default.find({ teamId }).sort({
            createdAt: -1,
        });
        res.json(assignments);
    }
...
async function getAssignmentStatus(req, res) {
    try {
        ...
        const activeAssignments = await AgentZoneAssignment_1.default.countDocuments({
            ...filter,
            status: { $nin: ["COMPLETED", "CANCELLED"] },
            effectiveTo: null,
        });
        // Get scheduled assignments
        const scheduledAssignments = await ScheduledAssignment_1.ScheduledAssignment.countDocuments({
            ...filter,
            status: "PENDING",
        });
        ...
        const completedThisWeek = await AgentZoneAssignment_1.default.countDocuments({
            ...filter,
            status: "COMPLETED",
            effectiveTo: { $gte: startOfWeek },
        });
        // Get pending approval (assignments that are inactive but not completed/cancelled)
        const pendingApproval = await AgentZoneAssignment_1.default.countDocuments({
            ...filter,
            status: "INACTIVE",
            effectiveTo: null,
        });
        ...
        const now = new Date();
        const overdueAssignments = await ScheduledAssignment_1.ScheduledAssignment.countDocuments({
            ...filter,
            status: "PENDING",
