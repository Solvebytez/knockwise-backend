...
export const createRouteValidation = [
  ...
  body("optimizationSettings.optimizationType")
    ...
    .withMessage(
      "Optimization type must be one of: FASTEST, SHORTEST, BALANCED"
    ),
  ...
  body("analytics.totalDistance")
    .optional()
    .isFloat({ min: 0 })
    .withMessage("Analytics total distance must be a positive number"),
  body("analytics.estimatedDuration")
    .optional()
    .isFloat({ min: 0 })
    .withMessage("Analytics estimated duration must be a positive number"),
];
...
export const updateRouteValidation = [
  ...
  body("status")
    ...
    .withMessage(
      "Status must be one of: DRAFT, PLANNED, IN_PROGRESS, COMPLETED, CANCELLED, ARCHIVED"
    ),
  ...
  body("stops.*.estimatedDuration")
    ...
    .isInt({ min: 1, max: 480 })
    .withMessage(
      "Each stop estimated duration must be between 1 and 480 minutes"
    ),
  body("stops.*.status")
    .optional()
    .isIn(["PENDING", "COMPLETED", "SKIPPED", "RESCHEDULED"])
    .withMessage(
      "Each stop status must be one of: PENDING, COMPLETED, SKIPPED, RESCHEDULED"
    ),
  body("stops.*.notes")
    .optional()
    .isString()
...
export const optimizeRouteValidation = [
  ...
  body("startLocation")
    ...
    .isArray({ min: 2, max: 2 })
    .withMessage(
      "Start location must be an array of 2 numbers [longitude, latitude]"
    ),
  body("startLocation.*")
    .optional()
    .isFloat()
  ...
  body("endLocation")
    ...
    .isArray({ min: 2, max: 2 })
    .withMessage(
      "End location must be an array of 2 numbers [longitude, latitude]"
    ),
  body("endLocation.*")
    .optional()
    .isFloat()
...
export const updateStopStatusValidation = [
  ...
  body("status")
    .isIn(["PENDING", "COMPLETED", "SKIPPED", "RESCHEDULED"])
    .withMessage(
      "Status must be one of: PENDING, COMPLETED, SKIPPED, RESCHEDULED"
    ),
  body("actualDuration")
    .optional()
    .isInt({ min: 1, max: 480 })
