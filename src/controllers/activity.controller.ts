...
export async function updateActivity(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    const updatedActivity = await Activity.findByIdAndUpdate(
      ...
      updateData,
      { new: true }
    ).populate('propertyId').populate('zoneId', 'name').populate('agentId', 'name email');
...
export async function getActivityStatistics(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    const dailyStats: Record<string, { total: number; duration: number; responses: Record<VisitResponse, number> }> = {};
...
export async function getAgentPerformance(req: AuthRequest, res: Response): Promise<void> {
  try {
