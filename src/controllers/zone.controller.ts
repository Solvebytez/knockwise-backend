...
const updateAgentStatus = async (agentId: string) => {
  try {
    ...
    if (!agent || agent.role !== "AGENT") {
      console.log(
        `❌ updateAgentStatus: Agent ${agentId} not found or not an agent`
      );
      return;
    }

    console.log(
      `📋 updateAgentStatus: Current status for ${(agent as any).firstName} ${
        (agent as any).lastName
      }: ${agent.status}`
    );
    console.log(
      `📋 updateAgentStatus: Agent zoneIds: [${agent.zoneIds.join(", ")}]`
    );
    console.log(
      `📋 updateAgentStatus: Agent primaryZoneId: ${
        agent.primaryZoneId || "None"
      }`
    );
    console.log(
      `📋 updateAgentStatus: Agent teamIds: [${agent.teamIds.join(", ")}]`
    );
    ...
    console.log(
      `📋 updateAgentStatus: Has individual zone assignment: ${hasIndividualZoneAssignment}`
    );
    console.log(
      `📋 updateAgentStatus: Has individual primary zone: ${hasIndividualPrimaryZone}`
    );
    console.log(
      `📋 updateAgentStatus: Has active individual zone assignment: ${hasActiveIndividualZoneAssignment}`
    );
    console.log(
      `📋 updateAgentStatus: Has team zone assignment: ${hasTeamZoneAssignment}`
    );
    console.log(
      `📋 updateAgentStatus: Has pending individual scheduled assignment: ${hasPendingIndividualScheduledAssignment}`
    );
    console.log(
      `📋 updateAgentStatus: Has pending team scheduled assignment: ${hasPendingTeamScheduledAssignment}`
    );
    ...
    console.log(
      `📋 updateAgentStatus: Should be active: ${shouldBeActive}, Calculated status: ${calculatedStatus}`
    );
    console.log(
      `📋 updateAgentStatus: Current status: ${agent.status}, New status: ${calculatedStatus}`
    );
    ...
    if (calculatedStatus === "ACTIVE" && agent.status !== "ACTIVE") {
      ...
      console.log(
        `✅ updateAgentStatus: Agent ${(agent as any).firstName} ${
          (agent as any).lastName
        } (${agentId}) status updated to ${calculatedStatus}`
      );
    } else {
      console.log(
        `✅ updateAgentStatus: Agent ${(agent as any).firstName} ${
          (agent as any).lastName
        } (${agentId}) status unchanged: ${agent.status}`
      );
    }
  } catch (error) {
  ...
  try {
    ...
    if (teamId) {
      ...
      if (team && team.agentIds && team.agentIds.length > 0) {
        ...
        const isFutureAssignment = effectiveDate > now;
        console.log(
          "📅 createZone: Assignment type:",
          isFutureAssignment ? "FUTURE" : "IMMEDIATE"
        );

        if (isFutureAssignment) {
          // Create scheduled assignment for future date
          const scheduledAssignment =
            await ScheduledAssignmentService.createScheduledAssignment({
              teamId: teamId,
...
export const listZones = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Skip role-based filtering for visualization purposes (to show all territories for overlap prevention)
    ...
    const zonesWithAssignments = await Promise.all(
      zones.map(async (zone) => {
        ...
        // Determine current assignment - show BOTH individual and team assignments when they exist
        let currentAssignment = null;

        if (activeAssignments.length > 0) {
          console.log(
            `🔍 Zone ${zone._id} (${zone.name}): Found ${activeAssignments.length} active assignments`
          );
          activeAssignments.forEach((assignment, index) => {
            console.log(
              `  Assignment ${index + 1}: agentId=${
                assignment.agentId
              }, teamId=${assignment.teamId}, status=${assignment.status}`
            );
          });

          // Find team assignment if exists
          const teamAssignment = activeAssignments.find(
            (assignment) => assignment.teamId
          );

          // Find individual assignment if exists
          const individualAssignment = activeAssignments.find(
            (assignment) => assignment.agentId && !assignment.teamId
          );

          console.log(
            `  Team assignment found: ${teamAssignment ? "YES" : "NO"}`
          );
          console.log(
            `  Individual assignment found: ${
              individualAssignment ? "YES" : "NO"
            }`
          );

          // Create assignment object that can show BOTH individual and team assignments
          if (individualAssignment && teamAssignment) {
            // Zone has BOTH individual and team assignments - show both
            console.log(
              `  ✅ Zone has BOTH assignments - showing individual agent AND team`
            );
            currentAssignment = {
              _id: individualAssignment._id, // Use individual assignment ID as primary
              agentId: individualAssignment.agentId, // Show individual agent
              teamId: teamAssignment.teamId, // Show team assignment
          ...
          } else if (individualAssignment) {
            // Only individual assignment
            console.log(
              `  ✅ Showing INDIVIDUAL assignment only for agent ${individualAssignment.agentId}`
            );
            currentAssignment = {
              _id: individualAssignment._id,
              agentId: individualAssignment.agentId,
              teamId: null,
              effectiveFrom: individualAssignment.effectiveFrom,
              effectiveTo: individualAssignment.effectiveTo,
              status: individualAssignment.status,
            ...
            // Only team assignment
            console.log(
              `  ✅ Showing TEAM assignment only for team ${teamAssignment.teamId}`
            );
            currentAssignment = {
            ...
            // Fallback to first assignment (shouldn't happen with current logic)
            console.log(`  ⚠️ Fallback to first assignment`);
...
export const checkZoneOverlapBeforeCreate = async (
  req: AuthRequest,
  res: Response
) => {
  try {
    ...
    if (
      ...
      duplicateAddresses = await checkDuplicateBuildings(
        buildingData.addresses,
        excludeZoneId
      );
    }
...
export const getZoneById = async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.log(
      `   Zone Created By Type: ${typeof zone.createdBy?.toString()}`
    );
    console.log(`   User ID Type: ${typeof req.user?.id}`);
    console.log(`   Is SUPERADMIN: ${req.user?.role === "SUPERADMIN"}`);
    console.log(
      `   Zone created by user: ${
        zone.createdBy?._id?.toString() === req.user?.id
      }`
    );
    console.log(
      `   Direct comparison: ${zone.createdBy?._id?.toString()} === ${
        req.user?.id
      }`
    );
    console.log(`   Zone createdBy._id: ${zone.createdBy?._id}`);
    console.log(
      `   Zone createdBy._id.toString(): ${zone.createdBy?._id?.toString()}`
    );
    console.log(
      `   Zone createdBy._id === User ID: ${
        zone.createdBy?._id?.toString() === req.user?.id
      }`
    );
    ...
    if (req.user?.role !== "SUPERADMIN") {
      // Check if user has access to this zone
      const hasAccess =
        zone.createdBy?._id?.toString() === req.user?.id || // Zone creator
    ...
    zoneData.currentAssignment = currentAssignment
      ? {
          ...
          effectiveFrom: currentAssignment.effectiveFrom,
          effectiveTo:
            "effectiveTo" in currentAssignment
...
export const updateZone = async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.log(
      `  Current Assigned Agent ID: ${zone.assignedAgentId || "None"}`
    );
    ...
    try {
      ...
      if (buildingData && buildingData.addresses && buildingData.coordinates) {
        ...
        if (
          ...
          await Resident.insertMany(residents);
          console.log(
            `✅ Created ${residents.length} residents with house numbers`
          );
        }
      }
      ...
      console.log(
        `✅ Deactivated ${deactivatedAssignments.modifiedCount} AgentZoneAssignment records`
      );
      ...
      console.log(
        `✅ Cancelled ${cancelledScheduled.modifiedCount} ScheduledAssignment records`
      );
      ...
      console.log(
        `📋 Is boundary update only: ${isBoundaryUpdateOnly || false}`
      );
      console.log(
        `📋 Is name/description update only: ${
          isNameDescriptionUpdateOnly || false
        }`
      );
      ...
      if (isBoundaryUpdateOnly) {
        ...
        if (zone.assignedAgentId) {
          ...
          const team = await Team.findById(zone.teamId);
          if (team) {
            console.log(
              `📋 Team: ${team.name} with ${team.agentIds.length} members`
            );
            ...
            if (isFutureDate) {
              ...
              await ScheduledAssignment.insertMany(scheduledAssignments);
              updateData.status = "SCHEDULED";
              console.log(
                `✅ Created ${scheduledAssignments.length} scheduled assignments for team`
              );
            } else {
              ...
              await AgentZoneAssignment.updateMany(
                { zoneId: id, teamId: zone.teamId, status: "ACTIVE" },
                { effectiveFrom: effectiveDate }
              );
        ...
        if (zone.status === "DRAFT" && !zone.assignedAgentId && !zone.teamId) {
          console.log(
            "🎯 SPECIAL CASE: Zone was previously unassigned (DRAFT status)"
          );
        ...
        if (zone.teamId) {
          console.log(
            "🔄 Zone was previously assigned to team, cleaning up old assignments..."
          );
          ...
          if (previousTeam) {
            console.log(
              `🔄 Found previous team: ${previousTeam.name} (${previousTeam._id})`
            );
            console.log(
              `📋 Previous team members: [${previousTeam.agentIds.join(", ")}]`
            );

            // Check if the new individual agent is part of the same team
            const isNewAgentInSameTeam = previousTeam.agentIds.some(
              (agentId) => agentId.toString() === assignedAgentId
            );
            console.log(
              `🔍 Is new agent in same team? ${isNewAgentInSameTeam}`
            );

            if (isNewAgentInSameTeam) {
              console.log(
                "🎯 SPECIAL CASE: New individual agent is a member of the same team!"
              );
              console.log(
                "📋 This means: Team assignment → Individual assignment (same team member)"
              );
              console.log(
                "📋 Other team members will become UNASSIGNED, but this agent keeps the assignment"
              );
            }

            console.log(
              `🔄 Cleaning up assignments for team ${previousTeam.name} (${previousTeam._id})...`
            );
            ...
            console.log(
              `✅ Deleted ${deletedZoneAssignments.deletedCount} old team zone assignments`
            );

            // 2. Remove old scheduled assignments for this zone (both team and individual)
            console.log(
              "🗑️ Step 2: Removing old scheduled assignments for this zone..."
            );
            const deletedScheduled = await ScheduledAssignment.deleteMany({
              zoneId: id,
            });
            console.log(
              `✅ Deleted ${deletedScheduled.deletedCount} old scheduled assignments for this zone`
            );
            ...
            console.log(
              `📋 Calling updateTeamStatus for team ${previousTeam._id}...`
            );
            await updateTeamStatus((previousTeam._id as any).toString());
            console.log(
              `📋 Calling updateTeamAssignmentStatus for team ${previousTeam._id}...`
            );
            await updateTeamAssignmentStatus(
              (previousTeam._id as any).toString()
            );
            ...
            console.log(
              `📋 Team assignment status will be determined by sync functions`
            );
            ...
            if (previousTeam.agentIds && previousTeam.agentIds.length > 0) {
              for (const agentId of previousTeam.agentIds) {
                ...
                if (agentId.toString() === assignedAgentId) {
                  console.log(
                    `⏭️ Skipping new assigned agent ${agentId} (will be updated later)`
                  );
                  continue;
                }
                ...
                console.log(
                  `✅ Removed zone ${id} from team member ${agentId}`
                );
              }
            }
            ...
            if (previousTeam.agentIds && previousTeam.agentIds.length > 0) {
              console.log(
                `🔄 Step 5: Updating ${previousTeam.agentIds.length} team members...`
              );
              for (const agentId of previousTeam.agentIds) {
                ...
                if (
                  isNewAgentInSameTeam &&
                  agentId.toString() === assignedAgentId
                ) {
                  console.log(
                    `    ⏭️ Skipping new assigned agent ${agentId} (will be updated later with new assignment)`
                  );
                  continue;
                }

                console.log(
                  `    🔄 Calling syncAgentZoneIds for agent ${agentId}...`
                );
                await syncAgentZoneIds(agentId.toString());
                console.log(
                  `    🔄 Calling updateUserAssignmentStatus for agent ${agentId}...`
                );
                await updateUserAssignmentStatus(agentId.toString());
              ...
              if (isNewAgentInSameTeam) {
                console.log(
                  `🎯 SPECIAL CASE: New agent ${assignedAgentId} is in same team, will be updated with new individual assignment`
                );
              }
            }
          } else {
        ...
        if (
          zone.assignedAgentId &&
          zone.assignedAgentId.toString() !== assignedAgentId
        ) {
          console.log(
            "🔄 Zone was previously assigned to individual agent, cleaning up old assignment..."
          );
          const previousAgent = await User.findById(zone.assignedAgentId);
          if (previousAgent) {
            console.log(
              `🔄 Cleaning up assignment for agent ${
                (previousAgent as any).firstName
              } ${(previousAgent as any).lastName} (${previousAgent._id})...`
            );
            ...
            console.log(
              `✅ Deleted ${deletedZoneAssignments.deletedCount} old individual zone assignments`
            );
            ...
            console.log(
              `🔄 Updating previous agent ${(previousAgent as any).firstName} ${
                (previousAgent as any).lastName
              } status...`
            );
            await syncAgentZoneIds(zone.assignedAgentId.toString());
            await updateUserAssignmentStatus(zone.assignedAgentId.toString());
            console.log(
              `✅ Updated status for previous agent ${
                (previousAgent as any).firstName
              } ${(previousAgent as any).lastName}`
            );
            ...
            if (previousAgent.teamIds && previousAgent.teamIds.length > 0) {
              console.log(
                `🔄 Previous agent was part of ${previousAgent.teamIds.length} team(s), checking team status...`
              );
        ...
        console.log(
          `✅ Agent found: ${(agent as any).firstName} ${
            (agent as any).lastName
          }`
        );
        ...
        console.log(
          `📋 Agent current assignment status: ${agent.assignmentStatus}`
        );

        // 3. Check if this is a future assignment or immediate assignment
        console.log("⏰ Step 6: Determining assignment timing...");
        console.log(
          `📋 Effective From provided: ${
            effectiveFrom || "None (using current date)"
          }`
        );
        ...
        console.log(`📋 Current Time: ${now}`);
        console.log(
          `📋 Time difference: ${effectiveDate.getTime() - now.getTime()}ms`
        );
        console.log(`📋 Is Future Assignment: ${isFutureAssignment}`);
        console.log(
          `⏰ Assignment timing: ${
            isFutureAssignment ? "FUTURE (SCHEDULED)" : "IMMEDIATE"
          }`
        );
        ...
        if (isFutureAssignment) {
          ...
          const newScheduledAssignment = await ScheduledAssignment.create(
            scheduledAssignmentData
          );
          console.log(
            `✅ Created scheduled assignment: ${newScheduledAssignment._id}`
          );
          ...
          const newAssignment = await AgentZoneAssignment.create(
            assignmentData
          );
          ...
          if (zone.status === "DRAFT") {
            ...
            console.log(
              "📋 Zone was not in DRAFT status, keeping current status..."
            );
            updateData.assignedAgentId = assignedAgentId;
            updateData.teamId = null;
          }
        ...
        console.log(
          `📋 Updating user ${assignedAgentId} with new zone assignment...`
        );
        ...
        if (hadTeamAssignment) {
          console.log(
            "🎯 FORCE CLEANUP: Team assignment detected (current or cancelled)!"
          );
          ...
          if (zone.teamId) {
            ...
            console.log(
              `📋 Previous team from zone.teamId: ${
                previousTeam?.name || "Not found"
              }`
            );
          } else {
            ...
            if (cancelledTeamAssignment?.teamId) {
              ...
              console.log(
                `📋 Previous team from cancelled assignment: ${
                  (previousTeam as any).name
                }`
              );
            }
          }
          ...
          if (previousTeam) {
            console.log(
              `🔧 FORCE CLEANUP: Found previous team: ${
                (previousTeam as any).name
              }`
            );
            ...
            console.log(
              `🔍 FORCE CLEANUP: Team ${
                (previousTeam as any).name
              } has active assignments: ${hasActiveAssignments}, pending scheduled: ${hasPendingScheduled}`
            );

            if (!hasActiveAssignments && !hasPendingScheduled) {
              await Team.findByIdAndUpdate((previousTeam as any)._id, {
                assignmentStatus: "UNASSIGNED",
              });
              console.log(
                `🔧 FORCE: Set team ${
                  (previousTeam as any).name
                } to UNASSIGNED (no active assignments to ANY zones)`
              );
            } else {
              console.log(
                `⚠️ FORCE CLEANUP: Team ${
                  (previousTeam as any).name
                } still has assignments to other zones, keeping as ASSIGNED`
              );
            }

            // FORCE: Set other team members to UNASSIGNED (only if they have no other assignments)
            if (
              (previousTeam as any).agentIds &&
              (previousTeam as any).agentIds.length > 0
            ) {
              for (const agentId of (previousTeam as any).agentIds) {
                if (agentId.toString() !== assignedAgentId) {
                  ...
                  console.log(
                    `🔍 FORCE CLEANUP: Team member ${agentId} has active assignments: ${hasActiveAssignments}, pending scheduled: ${hasPendingScheduled}`
                  );
                  ...
                  if (!hasActiveAssignments && !hasPendingScheduled) {
                    ...
                    console.log(
                      `🔧 FORCE: Set team member ${agentId} to UNASSIGNED (no active assignments to ANY zones)`
                    );
                  } else {
                    console.log(
                      `⚠️ FORCE CLEANUP: Team member ${agentId} still has assignments to other zones, keeping as ASSIGNED`
                    );
                  }
                }
              }
            ...
            console.log(
              "🔍 FORCE CLEANUP VERIFICATION: Checking final status..."
            );
            const updatedTeam = await Team.findById((previousTeam as any)._id);
            if (updatedTeam) {
              console.log(
                `📋 Team ${updatedTeam.name} final assignmentStatus: ${updatedTeam.assignmentStatus}`
              );
            }
            ...
            for (const agentId of (previousTeam as any).agentIds) {
              if (agentId.toString() !== assignedAgentId) {
                ...
                if (updatedAgent) {
                  console.log(
                    `📋 Team member ${agentId} final assignmentStatus: ${updatedAgent.assignmentStatus}`
                  );
                  console.log(
                    `📋 Team member ${agentId} final zoneIds: [${updatedAgent.zoneIds.join(
                      ", "
                    )}]`
                  );
                }
        ...
        console.log(
          "🔄 Cleaning up previous assignments for team assignment..."
        );

        // Scenario 1: Zone was previously assigned to a different team
        if (zone.teamId && zone.teamId.toString() !== teamId) {
          console.log(
            "🔄 Zone was previously assigned to different team, cleaning up old assignments..."
          );
          const previousTeam = await Team.findById(zone.teamId);
          if (previousTeam) {
            console.log(
              `🔄 Cleaning up assignments for previous team ${previousTeam.name} (${previousTeam._id})...`
            );
            ...
            console.log(
              `✅ Deleted ${deletedScheduled.deletedCount} old scheduled team assignments`
            );

            // 3. Now update previous team status (after removing old assignments)
            console.log(
              `🔄 Updating previous team ${previousTeam.name} (${previousTeam._id}) status...`
            );
            await updateTeamStatus((previousTeam._id as any).toString());
            await updateTeamAssignmentStatus(
              (previousTeam._id as any).toString()
            ...
            console.log(
              `✅ Updated status for previous team ${previousTeam.name}`
            );

            // 4. Update all previous team members' status and clear zone assignments
            if (previousTeam.agentIds && previousTeam.agentIds.length > 0) {
              console.log(
                `🔄 Updating ${previousTeam.agentIds.length} previous team members...`
              );
              for (const agentId of previousTeam.agentIds) {
                ...
                console.log(
                  `    ✅ Updated status for previous team member ${agentId}`
                );
              }
            }
          }
        ...
        if (zone.assignedAgentId) {
          ...
          if (previousAgent) {
            console.log(
              `🔄 Cleaning up assignment for previous agent ${
                (previousAgent as any).firstName
              } ${(previousAgent as any).lastName} (${previousAgent._id})...`
            );
        ...
        const isFutureAssignment = effectiveDate > now;
        console.log(
          `⏰ Assignment timing: ${isFutureAssignment ? "FUTURE" : "IMMEDIATE"}`
        );
        console.log(`Effective Date: ${effectiveDate}`);
        console.log(`Current Time: ${now}`);
      ...
      if (assignedAgentId) {
        ...
        if (agent) {
          console.log(
            `✅ primaryZoneId will be updated by updateUserAssignmentStatus for agent ${
              (agent as any).firstName
            } ${(agent as any).lastName}`
          );
        ...
        await updateTeamAssignmentStatus(teamId);
        console.log(
          `✅ Updated team status and assignment status for team ${teamId}`
        );
        ...
        if (team && team.agentIds) {
          ...
          for (const agentId of team.agentIds) {
            ...
            if (agent) {
              // The primaryZoneId will be handled by updateUserAssignmentStatus
              console.log(
                `    ✅ Updated primary zone for ${(agent as any).firstName} ${
                  (agent as any).lastName
                }`
              );
              ...
              await syncAgentZoneIds(agentId.toString());
              console.log(
                `    ✅ Synced zone IDs for ${(agent as any).firstName} ${
                  (agent as any).lastName
                }`
              );
            }
            ...
            await updateUserAssignmentStatus(agentId.toString());
            console.log(
              `    ✅ Updated assignment status for member ${agentId}`
            );
          }
        }
      } else if (removeAssignment) {
        ...
        console.log(
          `🔄 Found ${previouslyAssignedUsers.length} previously assigned users to update`
        );

        for (const user of previouslyAssignedUsers) {
          console.log(
            `  Processing user: ${(user as any).firstName} ${
          ...
          console.log(
            `    ✅ Updated status for ${(user as any).firstName} ${
              (user as any).lastName
            }`
          );
        }
      ...
      if (finalZone.buildingData?.houseStatuses) {
        const houseStatuses = Array.from(
          (finalZone.buildingData.houseStatuses as any).values()
        );
        const totalHouses = houseStatuses.length;
        const visitedHouses = houseStatuses.filter(
          (house: any) => house.status !== "not-visited"
      ...
      if (zoneData.currentAssignment) {
        ...
        console.log(
          `    Agent ID: ${zoneData.currentAssignment.agentId || "None"}`
        );
        console.log(
          `    Team ID: ${zoneData.currentAssignment.teamId || "None"}`
        );
        console.log(`    Status: ${zoneData.currentAssignment.status}`);
        console.log(
          `    Effective From: ${zoneData.currentAssignment.effectiveFrom}`
        );
      }
  ...
  try {
    ...
    allZoneAssignments.forEach((assignment) => {
      if (assignment.agentId) {
        ...
        const agentId =
          typeof assignment.agentId === "object" && assignment.agentId._id
      ...
      if (assignment.teamId) {
        // Handle both populated objects and ObjectIds
        const teamId =
          typeof assignment.teamId === "object" && assignment.teamId._id
    ...
    console.log(
      "👥 deleteZone: Affected agent IDs:",
      Array.from(affectedAgentIds)
    );
    console.log(
      "👥 deleteZone: Affected team IDs:",
    ...
    try {
      ...
      console.log(
        `Deleted ${deletedZoneAssignments.deletedCount} agent zone assignments`
      );
      ...
      console.log(
        `Deleted ${deletedScheduled.deletedCount} scheduled assignments`
      );
      ...
      console.log(
        `Deleted ${deletedResidents.deletedCount} residents for zone ${id}`
      );
      ...
      console.log(
        "🔄 deleteZone: Starting status updates for affected users and teams..."
      );

      // Update assignment status and zoneIds for affected agents
      for (const agentId of affectedAgentIds) {
        console.log(
          "👤 deleteZone: Updating assignment status for agent:",
          agentId
        );
      ...
      for (const teamId of affectedTeamIds) {
        console.log(
          "👥 deleteZone: Updating assignment status for team:",
          teamId
        );
        ...
        console.log(
          "👥 deleteZone: Fetching team to update member statuses..."
        );
        const team = await Team.findById(teamId);
        if (team && team.agentIds && team.agentIds.length > 0) {
          console.log(
            `👥 deleteZone: Found ${team.agentIds.length} team members to update`
          );
          for (const agentId of team.agentIds) {
            console.log(
              `👤 deleteZone: Updating assignment status for team member: ${agentId}`
            );
            await updateUserAssignmentStatus(agentId.toString());

            // Also sync zoneIds for team members
            console.log(
              `👤 deleteZone: Syncing zoneIds for team member: ${agentId}`
            );
            await syncAgentZoneIds(agentId.toString());
          }
          console.log(
            "✅ deleteZone: All team members assignment status and zoneIds updated"
          );
        } else {
      ...
      let message =
        "Zone and all associated residential data deleted successfully";
      if (activeAssignments.length > 0) {
...
export const removeAgentFromZone = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const assignment = await AgentZoneAssignment.findById(
      assignmentId
...
export const getZoneResidents = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Check if user has access to this zone
    const hasAccess =
      req.user?.role === "SUPERADMIN" || // SUPERADMIN has access to all zones
      ...
      (req.user?.zoneIds &&
        req.user.zoneIds.includes((zone._id as any).toString())); // User is individually assigned to this zone
...
export const getTerritoryOverviewStats = async (
  req: AuthRequest,
  res: Response
) => {
  try {
    ...
    const zonesWithBuildings = await Zone.find(filter).select("buildingData");
    for (const zone of zonesWithBuildings) {
      if (zone.buildingData?.residentialHomes) {
        ...
        if (zone.buildingData.houseStatuses) {
          ...
          const visitedHouses = houseStatuses.filter(
            (house: any) =>
              house.status !== "not-visited" &&
              house.status !== "not-interested"
          ).length;
    ...
    const stats = {
      ...
      topPerformingTerritory: topPerformingTerritory
        ? {
            ...
            completionRate:
              (topPerformingTerritory as any).performance?.completionRate || 85,
          }
        : undefined,
    };
  ...
  try {
    ...
    const buildingStats = {
      ...
      oddBuildings: {
        ...
        range:
          (zone.buildingData as any)?.houseNumbers?.odd?.length > 0
            ? {
                min: Math.min(
                  ...((zone.buildingData as any).houseNumbers.odd || [])
                ),
                max: Math.max(
                  ...((zone.buildingData as any).houseNumbers.odd || [])
                ),
      ...
      evenBuildings: {
        ...
        range:
          (zone.buildingData as any)?.houseNumbers?.even?.length > 0
            ? {
                min: Math.min(
                  ...((zone.buildingData as any).houseNumbers.even || [])
                ),
                max: Math.max(
                  ...((zone.buildingData as any).houseNumbers.even || [])
                ),
...
export const getAllZonesBuildingStats = async (
  req: AuthRequest,
  res: Response
) => {
  try {
    ...
    const summary = {
      ...
      totalBuildings: zonesStats.reduce(
        (sum, zone) => sum + zone.totalBuildings,
        0
      ),
      totalOddBuildings: zonesStats.reduce(
        (sum, zone) => sum + zone.oddCount,
        0
      ),
      totalEvenBuildings: zonesStats.reduce(
        (sum, zone) => sum + zone.evenCount,
        0
      ),
      averageBuildingsPerZone:
        zonesStats.length > 0
          ? Math.round(
              zonesStats.reduce((sum, zone) => sum + zone.totalBuildings, 0) /
                zonesStats.length
            )
          : 0,
    };
...
export const getTerritoryMapView = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (currentUserRole === "SUPERADMIN") {
      // SUPERADMIN can access any zone
      console.log("✅ SUPERADMIN access granted to zone");
    } else if (currentUserRole === "SUBADMIN") {
      // SUBADMIN can only access zones they created
      if (zone.createdBy?._id?.toString() !== currentUserId) {
        console.log("🚫 SUBADMIN access denied - not the creator");
        return res.status(403).json({
          success: false,
          message: "Access denied to this zone",
        });
      }
      console.log("✅ SUBADMIN access granted - zone creator");
    } else if (currentUserRole === "AGENT") {
      // AGENT can access zones they are assigned to (individually or via team)
      ...
      // Check if agent has individual assignment
      const individualAssignment = await AgentZoneAssignment.findOne({
        zoneId: id,
      ...
      // Check if agent has team assignment
      const teamAssignment = await AgentZoneAssignment.findOne({
        zoneId: id,
        teamId: { $in: agent.teamIds },
        status: { $nin: ["COMPLETED", "CANCELLED"] },
        effectiveTo: null,
      ...
      // Check if agent has scheduled assignment
      const scheduledIndividualAssignment = await ScheduledAssignment.findOne({
        zoneId: id,
        agentId: currentUserId,
        status: "PENDING",
      });

      const scheduledTeamAssignment = await ScheduledAssignment.findOne({
        zoneId: id,
        teamId: { $in: agent.teamIds },
        status: "PENDING",
      });
      ...
      if (
        ...
        !teamAssignment &&
        !scheduledIndividualAssignment &&
        !scheduledTeamAssignment
      ) {
        console.log("🚫 AGENT access denied - not assigned to this zone");
        return res.status(403).json({
          success: false,
          message: "Access denied to this zone",
        });
      }
      console.log("✅ AGENT access granted - has assignment to zone");
    } else {
      return res.status(403).json({
        success: false,
        message: "Access denied to this zone",
  ...
  try {
    ...
    try {
      ...
      if (Object.keys(zoneUpdateData).length > 0) {
        const updatedZoneResult = await Zone.findByIdAndUpdate(
          id,
          zoneUpdateData,
          { new: true, session }
        ).populate("createdBy", "name email");
      ...
      if ((updateType === "residents" || updateType === "all") && residents) {
        ...
        if (residentData.length > 0) {
          ...
          console.log(
            `✅ Created ${residentData.length} new residents for zone:`,
            id
          );
        } else {
