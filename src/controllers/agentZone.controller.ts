...
/**
 * Update a manual zone for mobile (no boundary required)
 * This endpoint is specifically for AGENT role users updating manual zones
 * Only updates name, description, and location hierarchy
 */
export const updateMobileManualZone = async (
  ...
  try {
    ...
    if (zone.zoneType !== "MANUAL") {
      return res.status(400).json({
        success: false,
        message: "This endpoint is only for manual zones. Use PUT /agent-zones/:id for map zones.",
      });
    }
    ...
    if (communityId || municipalityId || areaId) {
      ...
      console.log("📱 updateMobileManualZone: Location hierarchy validated:", {
        area: area.name,
        municipality: municipality.name,
        community: community.name,
      });
    ...
    // Handle community change - update old and new community zone references
    const oldCommunityId = zone.communityId?.toString();
    const newCommunityId = communityId;

    if (oldCommunityId && newCommunityId && oldCommunityId !== newCommunityId) {
      // Remove zone from old community
      await Community.findByIdAndUpdate(oldCommunityId, {
        $pull: { zoneIds: zone._id },
      });
      console.log(
        "📱 updateMobileManualZone: Removed zone from old community:",
        oldCommunityId
      );

      // Add zone to new community
      await Community.findByIdAndUpdate(newCommunityId, {
        $addToSet: { zoneIds: zone._id },
      });
      console.log(
        "📱 updateMobileManualZone: Added zone to new community:",
        newCommunityId
      );
    } else if (!oldCommunityId && newCommunityId) {
      // Add zone to new community if not previously assigned
      await Community.findByIdAndUpdate(newCommunityId, {
        $addToSet: { zoneIds: zone._id },
      });
      console.log(
        "📱 updateMobileManualZone: Added zone to community:",
        newCommunityId
      );
    }
    ...
    try {
      ...
      if (areaId && areaId !== zone.areaId?.toString()) changes.push("area updated");
      if (municipalityId && municipalityId !== zone.municipalityId?.toString()) changes.push("municipality updated");
      if (communityId && communityId !== zone.communityId?.toString()) changes.push("community updated");
      ...
      await Activity.create({
        ...
        startedAt: new Date(),
        notes: changes.length > 0 ? `Manual zone updated: ${changes.join(", ")}` : "Manual zone updated",
      });
