...
const logTeamChange = async (action: string, teamId: string, details: any, currentUserId: string) => {
  try {
...
const calculateTeamStatus = async (teamId: string): Promise<'ACTIVE' | 'INACTIVE'> => {
  try {
...
const calculateAgentStatus = async (agentId: string): Promise<'ACTIVE' | 'INACTIVE'> => {
  try {
    const agent = await User.findById(agentId);
...
export const createTeam = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (memberIdsToUse && memberIdsToUse.length > 0) {
      ...
      await logTeamChange('CREATED', (team as any)._id.toString(), {
        name: team.name,
...
export const getTeamById = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const membersWithCorrectStatus = await Promise.all(team.agentIds.map(async (agent: any) => {
...
export const updateTeam = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (addedMembers.length > 0 || removedMembers.length > 0) {
      ...
      if (addedMembers.length > 0) {
        ...
        if (teamZoneIds.length > 0) {
          ...
          if (newZoneAssignments.length > 0) {
            await AgentZoneAssignment.insertMany(newZoneAssignments, { session });
        ...
        for (const memberId of addedMembers) {
          ...
          if (agent) {
            ...
            const newZoneIds = teamZoneIds.filter(zoneId => 
              !existingZoneIds.includes(zoneId)
            );
        ...
        for (const memberId of addedMembers) {
          ...
          if (existingRoutes.length > 0) {
            ...
            await Route.updateMany(
              { agentId: memberId, teamId: { $exists: false }, status: { $in: ['DRAFT', 'PLANNED'] } },
              { teamId: teamId },
              { session }
            );
      ...
      if (removedMembers.length > 0) {
        ...
        if (scheduledAssignmentsToUpdate.length > 0) {
          ...
          const remainingMembers = newMemberIds.filter((id: any) => !removedMembers.includes(id));
    ...
    try {
      ...
      if (addedMembers.length > 0) {
        await Promise.all(addedMembers.map((memberId: string) => syncAgentZoneIds(memberId)));
        await Promise.all(addedMembers.map((memberId: string) => updateAgentStatus(memberId)));
        await Promise.all(addedMembers.map((memberId: string) => updateUserAssignmentStatus(memberId)));
      }
      
      if (removedMembers.length > 0) {
        await Promise.all(removedMembers.map((memberId: string) => syncAgentZoneIds(memberId)));
        await Promise.all(removedMembers.map((memberId: string) => updateAgentStatus(memberId)));
        await Promise.all(removedMembers.map((memberId: string) => updateUserAssignmentStatus(memberId)));
      }
      ...
      const remainingMembers = newMemberIds.filter((id: any) => 
        !addedMembers.includes(id) && !removedMembers.includes(id)
      );
      if (remainingMembers.length > 0) {
        await Promise.all(remainingMembers.map((memberId: string) => updateUserAssignmentStatus(memberId)));
      }
...
export const getTeamPerformance = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const performanceData = await Promise.all(teams.map(async (team) => {
      ...
      const activeMembers = await Promise.all(team.agentIds.map(async (agent: any) => {
        const calculatedStatus = await calculateAgentStatus(agent._id.toString());
